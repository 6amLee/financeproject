...
**Project:** Automated receipt intake \+ credit-card reconciliation ("The FinanceCrew") **Owner:** Lee (Head of People) · **Finance:** Yulia · **Escalation:** Roee · **API/billing:** Ron **Last updated:** end of the Make-intake build session (intake working; dedup \+ Claude body mid-fix)
...
- **The FinanceCrew** (statement matching + Slack chase, §10 below) is being designed and built directly in code from the start — see `FinanceCrew_Design_Doc.md` in this repo for the full spec, grounded in the real statement export and Vendor Ownership sheet.
...
- **The FinanceCrew** — a scheduled bot (not built yet) that will check the company credit-card statement against the master DB every 24h and chase missing receipts over Slack.

Refundly is **not** integrated — it only defined the column set. The Google Sheet is the single source of truth.

One-liner: *Email → Claude reads it → row in the Master DB. Later, The FinanceCrew checks the statement against the DB daily and chases the gaps.*
...
| The FinanceCrew | Bot (scheduled) | Matching \+ Slack chase | ⬜ not built |

**Skills vs bots vs brains:** The FinanceCrew is a **bot** (runs on a 24h trigger); Claude is a **brain** (logic, invoked by the bot); the Sheet is **data**. Make is the automation host that ties them together.
...
- The FinanceCrew (matching \+ Slack chase).  
...
- `Paid by` ∈ {Employee, Organization}. **Only Organization rows get reconciled** by The FinanceCrew. Employee \= reimbursement list for Yulia.  
...
### Escalation lists (for The FinanceCrew's chase)
...
## 10\. The FinanceCrew (matching \+ chase) — spec summary (not built)
...
8. **Build The FinanceCrew** — normalizer → matching → Slack chase (per §10).  
