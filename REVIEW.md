# Codebase Review — FinanceCrew Finance Bot
...
**Files:** `src/financeCrew/chase.js:228-245` · `src/slackIntake.js:18-30` (the helper module)
...
**Files:** `src/sheets.js:13-16` · `src/financeCrew/ledger.js:23-26` · `src/financeCrew/chaseState.js:25-28`
...
**Files:** `src/sheets.js:41-90` · `src/financeCrew/ledger.js:82-100` · `src/financeCrew/chaseState.js:80-91`
...
**Files:** `src/financeCrew/ledger.js:31-33` · `src/financeCrew/chaseState.js:33-35`

Both are `/^true$/i.test(String(v ?? "").trim())` with different names.

**Suggested fix:** Export `parseBool` from one module; the other imports it. Or move to a shared `src/financeCrew/utils.js`.
**Risk flag:** None.

---

### P1-5 · `normalizeMerchant` duplicated as a private `norm` in resolver.js
**Files:** `src/financeCrew/matcher.js:49-51` (exported) · `src/financeCrew/resolver.js:48` (private const)
...
They are identical. resolver.js already imports from matcher.js (transitively via financeCrew.js), so there's no impedance mismatch.
...
### P1-7 · `MASTER_DB_RANGE` in financeCrew.js stops at column O — misses column P
**File:** `financeCrew.js:64`

```js
const MASTER_DB_RANGE = "'Master DB'!A2:O";
```

Column P (`document_type`) was added after this line was written. FinanceCrew reads 15 columns and the 16th is always `undefined` for every Master DB row it processes. matcher.js `MASTER_COL` doesn't reference column P, so no crash — but it means FinanceCrew can never see `document_type` for future matching logic.
...
**File:** `src/financeCrew/matcher.js:33`
...
### [LOW] S2-L3 · FinanceCrew's `MASTER_DB_RANGE` stops at column O — `document_type` invisible to matcher
**File:** `financeCrew.js:64` *(also listed as P1-7 for cleanup)*

```js
const MASTER_DB_RANGE = "'Master DB'!A2:O";
```

This is both a cleanup issue and a mild data integrity concern: FinanceCrew performs matching decisions based on an incomplete view of each receipt row. Currently `document_type` is not used by the matcher, so there is no impact — but if a future matching rule needs "is this a receipt or an invoice?" the column will silently be `undefined`.
...
**Files:** `src/financeCrew/ledger.js` — exports `appendLedgerEntry` and `buildLedgerRow`; `financeCrew.js` — does not import them
