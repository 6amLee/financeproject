...
Decisions made this session: best-effort parse the messy Owner(s) column with a manual-review flag for anything that doesn't yield clean names (rather than requiring a sheet cleanup pass first); FinanceCrew runs as a **separate entry point** in the same `financeproject` repo (its own poll loop, not merged into the existing email-intake `index.js`, since intake polls every few minutes but chase cadence is measured in hours); **how the statement file reaches FinanceCrew is explicitly undecided** — do not build a specific ingestion path (neither a Drive-folder watch nor an email-based one) until that's settled. The normalizer is designed as a pure function that takes already-extracted file content in and returns flat rows out, so it's ready to plug into whichever ingestion method gets picked later.

## Architecture

New entry point `financeCrew.js` alongside the existing `index.js`, sharing the existing `src/googleAuth.js` singleton and following the same style as `src/sheets.js` (pure builder functions separate from I/O, promise-queue for concurrent Sheets writes). New modules under `src/financeCrew/`:

```
financeCrew.js                          — poll loop entry point (separate from index.js)
  └─ src/financeCrew/normalizer.js      — Stage 0: raw statement text/rows → flat transaction rows
  └─ src/financeCrew/ownership.js       — loads + best-effort-parses Vendor Ownership sheet
  └─ src/financeCrew/matcher.js         — Stage 1: match statement rows ↔ Master DB receipt rows
  └─ src/financeCrew/resolver.js        — Stage 2: rank who to nudge per unmatched charge
  └─ src/financeCrew/ledger.js          — learning ledger: read/write resolution history to a Sheet tab
  └─ src/financeCrew/chase.js           — Stage 3: cadence state machine + Slack message sending
  └─ src/sheets.js (extended)      — add ledger + chase-state read/write helpers
```

### Stage 0 — Normalizer (`src/financeCrew/normalizer.js`)
...
### Stage 1 — Matching (`src/financeCrew/matcher.js`)
...
### Stage 2 — Ownership resolution (`src/financeCrew/resolver.js` + `src/financeCrew/ledger.js`)
...
Every resolution (whether a receipt eventually gets matched to a person, or a chase gets a confirmed response) writes a row to a new Sheet tab, e.g. `FinanceCrew Ledger`: `vendor, card, resolved_owner, resolved_at, resolution_source (vendor_map|vendor_history|card_history|cold_start), confirmed (bool)`. This is the self-maintaining piece — no manual card-access list needed.

**Ownership sheet parsing** (`src/financeCrew/ownership.js`): split `Owner(s)` on commas, trim each fragment, drop fragments that don't look like a name (e.g. `"+ other people"`, empty strings) via a simple heuristic (fragment contains only letters/spaces, length ≥2, not a known filler phrase). Rows where this yields zero clean names get collected into a `needsReview` list returned alongside the parsed map — surfaced in FinanceCrew's own log/report rather than silently dropped or guessed.

### Stage 3 — Chase cadence (`src/financeCrew/chase.js`)

State machine per unmatched-charge-cluster, persisted in a Sheet tab (e.g. `FinanceCrew Chase State`): `cluster_id, vendor, amount, stage (1-4), stage_entered_at, last_nudge_at, resolved (bool)`.
...
- `financeCrew.js` — poll loop entry point, `--once` flag for local testing (mirrors `index.js`'s existing pattern).
- `src/financeCrew/normalizer.js` — pure function, Stage 0 logic above.
- `src/financeCrew/ownership.js` — load + best-effort parse Vendor Ownership sheet, return `{ map, needsReview }`.
- `src/financeCrew/matcher.js` — Stage 1 matching + clustering logic.
- `src/financeCrew/resolver.js` — Stage 2 ranking logic.
- `src/financeCrew/ledger.js` — read/write the FinanceCrew Ledger tab.
- `src/financeCrew/chase.js` — Stage 3 state machine + Slack nudge sending.
...
1. **Statement ingestion path** — explicitly deferred. Normalizer is built input-agnostic; a follow-up decision is needed on Drive-folder-watch vs email-forward vs something else before FinanceCrew can run unattended.
...
- Manual: once ingestion path is decided, run `node financeCrew.js --once` against a real statement + the current (small, 3-row) Master DB, inspect console output for match/review/missing classification before wiring any real Slack sends.
